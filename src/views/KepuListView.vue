  ...
  if (canShare.value) {
    try {
      const shareData = {
        title: article.title,
        text: article.title,
        url: "http://38.38.251.86:8000/kepuContent?id=" + article.id,
      }
  ...
  <div class="kepu-page-wrapper main-background">
    ...
    <div class="main">
      <el-scrollbar max-height="100%">
        ...
        <div v-else-if="loadError || kepuStore.error" class="error-container">
          <p>加载失败，请稍后再试</p>
          <el-button type="primary" @click="kepuStore.fetchKepu()">重新加载</el-button>
        </div>
        
        <div v-else-if="!kepuStore.kepuArticles || kepuStore.kepuArticles.length === 0" class="empty-container">
          <p>暂无科普内容</p>
        </div>
        
        <div v-else v-for="(article, index) in kepuStore.kepuArticles" :key="index" class="layout-single-image">
          <div class="layout-single-image-l">
            <div style="color:#333333;" @click="openLink(`/kepuContent?id=${article.id}`)">
              {{ article.title }}
            </div>
          ...
          <el-image 
            ...
            fit="cover" 
            class="image-content" 
            @click="openLink(`/kepuContent?id=${article.id}`)">
            <template #error>
